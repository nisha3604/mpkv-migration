...
import { mapUrlForEVC, isGroupItem } from '../utils/menuUrlMap'
import SiteHeader from './SiteHeader'
import SiteFooter from './SiteFooter'

/**
 * EVerificationLayout — layout for EVC users (UserTypeID 41/42).
 * Mirrors MasterPageWithSession.Master from old project.
 * Uses DB-driven menu via Menu_GetMenu SP (same as CollegeLayout).
 * Dashboard link is hardcoded first, then dynamic items from DB.
 */
export default function EVerificationLayout({ children }) {
  ...
  const dashPath = '/everification/dashboard'
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <nav ref={navRef} style={{ backgroundColor:'#14212e', padding:'0 24px', display:'flex', alignItems:'center', position:'relative', zIndex:40 }}>
        ...
        <ul style={{ display:'flex', flexDirection:'row', listStyle:'none', margin:0, padding:0, flex:1 }}>
              ...
              <li key={item.menuID} style={{ position:'relative' }}>
                ...
                {isOpen && kids.length > 0 && (
                  <div style={{ position:'absolute', top:'100%', left:0, zIndex:9999, background:'#fff', border:'1px solid #e2e8f0', borderRadius:8, boxShadow:'0 8px 24px rgba(0,0,0,.14)', minWidth:240, padding:'6px 0' }}>
                    {kids.map(child => {
                      const childUrl = mapUrlForEVC(child.linkURL)
          ...
          {/* Fallback static items if no DB menu exists for EVC */}
          {menuItems.length === 0 && (
            <>
              {[
                { label:'Candidate List',        to:'/everification/candidates' },
                { label:'Verify by Application', to:'/everification/check'      },
              ].map((item, i) => {
                const isAct = location.pathname === item.to
                return (
                  <li key={i}>
                    <Link to={item.to}
                      style={{ display:'flex', alignItems:'center', color: isAct ? '#fff' : 'rgba(255,255,255,.85)', textDecoration:'none', fontSize:14, fontWeight: isAct ? 600 : 500, padding:'12px 16px', whiteSpace:'nowrap', borderBottom: isAct ? '2px solid #059669' : '2px solid transparent' }}
                      onMouseEnter={e => { e.currentTarget.style.color='#fff'; e.currentTarget.style.background='rgba(255,255,255,.06)' }}
                      onMouseLeave={e => { e.currentTarget.style.color=isAct?'#fff':'rgba(255,255,255,.85)'; e.currentTarget.style.background='transparent' }}>
                      {item.label}
                    </Link>
                  </li>
                )
      ...
      {showLogout && (
        <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, backdropFilter:'blur(2px)' }}>
          <div style={{ background:'#fff', borderRadius:16, padding:'32px 36px', maxWidth:380, width:'90%', textAlign:'center', boxShadow:'0 20px 60px rgba(0,0,0,.3)' }}>
            ...
            <h3 style={{ fontSize:17, fontWeight:700, color:'#0f172a', margin:'0 0 8px' }}>Confirm Sign Out</h3>
            <p style={{ fontSize:14, color:'#64748b', margin:'0 0 22px' }}>Are you sure you want to sign out?</p>
            <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
              <button onClick={() => setShowLogout(false)}
                style={{ background:'#f1f5f9', border:'none', color:'#374151', padding:'9px 22px', borderRadius:8, fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                Cancel
              </button>
              <button onClick={handleLogout}
                style={{ background:'#ef4444', color:'#fff', border:'none', padding:'9px 22px', borderRadius:8, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
                Yes, Sign Out
              </button>
            </div>
          </div>
