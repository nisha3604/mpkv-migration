...
export default function AdminDashboard() {
  ...
  const links = [
    { label:'College List',           icon:'fa-list',          to:'/admin/college/list',           desc:'Search and manage all colleges' },
    { label:'College Passwords',      icon:'fa-key',           to:'/admin/college/passwords',       desc:'View and send college login passwords' },
    { label:'Reset College Password', icon:'fa-lock-open',     to:'/admin/college/reset-password',  desc:'Reset a college user password' },
  ]
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
        {links.map((l,i) => (
          <button key={i} onClick={() => navigate(l.to)}
            ...
            <div style={{ width:44, height:44, background:'#f0fdf4', borderRadius:10, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:12 }}>
              <i className={`fas ${l.icon}`} style={{ color:V.primary, fontSize:18 }} />
            </div>
