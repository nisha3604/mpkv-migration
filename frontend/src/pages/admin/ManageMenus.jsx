...
/**
 * ManageMenus — mirrors ManageMenus.aspx exactly.
 *
 * Functionality (same as old project):
 *  1. UserType dropdown — loads Group dropdown on change
 *  2. Group dropdown   — loads menu list on change
 *  3. "Add New Menu" button → /admin/menu/add-edit?MenuID=0&UserTypeID=X&ParentMenuID=Y
 *  4. Grid rows: Edit icon | Menu Name | Display Start | Display End | IsActive ☑ | Set Seq ☑ | Seq No input
 *  5. "Set Seq" checkbox: when checked → auto-increment SeqNo counter, highlight row green
 *     Unchecking → clears that seq and shifts others down (same JS logic as old)
 *  6. "Reset Menus" button → clears all Set Seq checkboxes + SeqNo inputs
 *  7. "Save Menus" button → calls reorderMenus SP with all rows (IsActive + SeqNo)
 *
...
export default function ManageMenus() {
  ...
  const [parentMenuId,  setParentMenuId]  = useState(sp.get('ParentMenuID') || '-1')
  const [groups,        setGroups]        = useState([])
  const [rows,          setRows]          = useState([])   // [{...item, checked, seqInput}]
  const [seqCounter,    setSeqCounter]    = useState(0)
  const [loading,       setLoading]       = useState(false)
  const [saving,        setSaving]        = useState(false)
  const [msg,           setMsg]           = useState({ text:'', ok:true })

  // Load groups when userType changes
  useEffect(() => {
    if (userTypeId === '-1') { setGroups([]); setRows([]); return }
    menuApi.getGroups(parseInt(userTypeId))
      .then(res => {
        const items = res.data.items ?? []
        setGroups(items)
        // if ParentMenuID from QS matches a group, keep it; else reset
        const valid = items.find(g => g.menuID.toString() === parentMenuId)
        if (!valid) setParentMenuId('-1')
      })
      .catch(() => {})
  }, [userTypeId])

  // Load menu list when group changes
  useEffect(() => {
    if (userTypeId === '-1' || parentMenuId === '-1') { setRows([]); return }
    setLoading(true)
    menuApi.getMenusList(parseInt(userTypeId), parseInt(parentMenuId))
      .then(res => {
        const items = res.data.items ?? []
        // Mirror gvMenusList_RowDataBound: if SeqNo > 0 → check the box; compute CheckSeqCount
        let maxSeq = 0
        const mapped = items.map(item => {
          const hasSeq = item.seqNo > 0
          if (hasSeq && item.seqNo > maxSeq) maxSeq = item.seqNo
          return { ...item, checked: hasSeq, seqInput: hasSeq ? item.seqNo.toString() : '' }
        })
        setRows(mapped)
        setSeqCounter(maxSeq)
        if (items.length === 0) setMsg({ text:'No Records Found.', ok:false })
        else setMsg({ text:'', ok:true })
      })
      .catch(() => setMsg({ text:'Failed to load menus.', ok:false }))
      .finally(() => setLoading(false))
  }, [userTypeId, parentMenuId])

  // Mirror SetSeqs(chkBox) — toggle checkbox + counter + highlight
  const handleSetSeq = (menuID, checked) => {
    setRows(prev => {
      ...
      const updated = prev.map(r => {
        ...
        if (checked) {
          counter++
          return { ...r, checked: true, seqInput: counter.toString() }
        } else {
          const removedSeq = parseInt(r.seqInput) || 0
          // Shift down rows with seqNo > removedSeq
          return { ...r, checked: false, seqInput: '' }
        }
      })
      // If unchecking — shift down all seqNo > removed
      if (!checked) {
        const target = prev.find(r => r.menuID === menuID)
        const removed = parseInt(target?.seqInput) || 0
        counter = Math.max(0, counter - 1)
        updated.forEach(r => {
          if (r.checked && parseInt(r.seqInput) > removed) {
            r.seqInput = (parseInt(r.seqInput) - 1).toString()
          }
        })
      }
  ...
  // Reset — clear all Set Seq checkboxes + SeqNo inputs (mirrors ResetSeqs())
  const handleReset = () => {
    setRows(prev => prev.map(r => ({ ...r, checked: false, seqInput: '' })))
    setSeqCounter(0)
  }

  // Save — calls reorderMenus with IsActive + SeqNo for every row
  const handleSave = async () => {
    ...
    try {
      const res = await menuApi.reorderMenus({
        items: rows.map(r => ({
          menuID:   r.menuID,
          seqNo:    r.seqInput.trim() !== '' ? parseInt(r.seqInput) : 0,
          isActive: r.isActive,
        }))
      })
      if (res.data.success) {
        ...
        // Reload
        const reload = await menuApi.getMenusList(parseInt(userTypeId), parseInt(parentMenuId))
        ...
        setSeqCounter(maxSeq)
      } else setMsg({ text: res.data.message || 'Failed to save.', ok:false })
  ...
  const thS = {
    padding:'10px 12px', color:'#fff', fontWeight:700, fontSize:11,
  ...
  return (
    <div style={{ fontFamily:'inherit', background:'#f1f5f9', minHeight:'100vh', padding:24 }}>
      ...
      <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderTop:'none', borderRadius:'0 0 12px 12px', padding:24 }}>
        ...
        {/* Filters row — mirrors 2 dropdowns centered */}
        <div style={{ display:'flex', justifyContent:'center', gap:24, marginBottom:20, flexWrap:'wrap' }}>
          <div>
            ...
            <select value={userTypeId}
              onChange={e => { setUserTypeId(e.target.value); setParentMenuId('-1'); setRows([]) }}
              style={selStyle}>
          ...
          <div>
            ...
            <select value={parentMenuId}
              onChange={e => setParentMenuId(e.target.value)}
              style={selStyle}>
              <option value="-1">Select</option>
              {groups.map(g => <option key={g.menuID} value={g.menuID}>{g.linkName}</option>)}
            </select>
          </div>
        </div>

        {/* Add New Menu button — centered */}
        {userTypeId !== '-1' && parentMenuId !== '-1' && (
          <div style={{ textAlign:'center', marginBottom:20 }}>
            <button
              onClick={() => navigate(`/admin/menu/add-edit?MenuID=0&UserTypeID=${userTypeId}&ParentMenuID=${parentMenuId}`)}
              ...
              <i className="fas fa-plus"/>Add New Menu
            </button>
          </div>
        )}
          ...
          <>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                <thead>
                  <tr style={{ background:'#14212e' }}>
                    ...
                    <th style={{ ...thS, width:'39%', textAlign:'left' }}>Menu Name</th>
                ...
                <tbody>
                  {rows.map((row, idx) => (
                    <tr key={row.menuID}
                      style={{ background: row.checked ? '#d4edda' : idx%2===0?'#fff':'#f9f9f9', transition:'background .15s' }}>
                      {/* Edit icon */}
                      <td style={tdS}>
                        <button
                          onClick={() => navigate(`/admin/menu/add-edit?MenuID=${row.menuID}&UserTypeID=${userTypeId}&ParentMenuID=${parentMenuId}`)}
                          style={{ background:'transparent', border:'none', cursor:'pointer', color:'#059669', fontSize:18, padding:0 }}
                          title="Edit">
                          <i className="fa fa-edit"/>
                        </button>
                      </td>
                      {/* Menu Name */}
                      <td style={{ ...tdS, textAlign:'left', fontWeight:500, color:'#0f172a' }}>
                        {row.linkName}
                      </td>
                      {/* Display Start */}
                      ...
                      <td style={tdS}>
                        <input type="checkbox" checked={row.isActive}
                          onChange={e => handleIsActive(row.menuID, e.target.checked)}
            ...
            <div style={{ textAlign:'center' }}>
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin inline-block"/>Saving...</> : <>Save Menus</>}
              </button>
            </div>
          </>
        )}
