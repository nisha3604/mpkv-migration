...
/**
 * EVCApplicationFormPrint — same layout as ApplicationFormPrint.jsx
 * but fetches data via admin API using Application ID from URL param.
 * Used by EVC users for "Print Application Form" action.
 * Route: /everification/print/:applicationId
 * No layout wrapper — raw print page, same as candidate version.
 */
...
export default function EVCApplicationFormPrint() {
  ...
  return (
    <>
      ...
      <table>
        ...
        <tr>
          <td style={tdr}>Father's Name</td><td style={tdv}>{p.fatherName}</td>
          <td style={tdr}>Mother's Name</td><td style={tdv}>{p.motherName}</td>
        </tr>
        <tr>
          <td style={tdr}>Gender</td><td style={tdv}>{p.gender}</td>
          <td style={tdr}>Date of Birth</td><td style={tdv}>{p.dob}</td>
        </tr>
        <tr>
          <td style={tdr}>Age (as on 01/07/2026)</td><td colSpan={3} style={tdv}>{p.age}</td>
        </tr>
        <tr>
          <td style={tdr}>E-Mail ID</td><td style={tdv}>{p.emailID}</td>
          <td style={tdr}>Mobile Number</td><td style={tdv}>{p.mobileNo}</td>
        </tr>
        <tr>
          <td style={tdr}>Are You a Resident of India?</td><td colSpan={4} style={tdv}>{p.isResidentOfIndia}</td>
        </tr>
        <tr><td style={tdr}>Permanent Address</td><td colSpan={4} style={tdv}>{permAddr}</td></tr>
        <tr><td style={tdr}>Correspondence Address</td><td colSpan={4} style={tdv}>{corrAddr}</td></tr>

        {/* ── Category ──────────────────────────────────────────── */}
        <tr><th colSpan={5} style={th}>Category &amp; Other Reservation Details</th></tr>
        <tr>
          <td style={tdr}>Father's Domicile District</td><td style={tdv}>{cat.domicileDistrict}</td>
          <td style={tdr}>Father's Domicile Village</td><td colSpan={2} style={tdv}>{cat.domicileVillage}</td>
        </tr>
        <tr>
          <td style={tdr}>Category</td><td style={tdv}>{cat.category} ({cat.caste})</td>
        ...
        {showNCLRcp   && <tr><td colSpan={4} style={tdr}>Do You have NCL Certificate Receipt?</td><td style={tdv}>{cat.hasNCLReceipt}</td></tr>}
        {showEWS      && <tr><td colSpan={4} style={tdr}>Do You have EWS Certificate?</td><td style={tdv}>{cat.hasEWSCertificate}</td></tr>}
        {[
          ...
          ['ज्याचे उत्पन्नाचे साधन शेतीच आहे अशा शेतकऱ्याचा मुलगा / मुलगी आहे का ?', cat.isIncomeSourceAgriculture],
          ['शेती धारण केली आहे काय ?', cat.hasFarm],
          ...
          ['खेळात / सांस्कृतिक कार्यक्रमात भाग घेतला आहे काय ?', cat.isSports],
        ].map(([lbl, val], i) => (
          <tr key={i}><td colSpan={4} style={tdr}>{lbl}</td><td style={tdv}>{val}</td></tr>
        ))}

        {/* ── Qualification ─────────────────────────────────────── */}
        <tr><th colSpan={5} style={th}>Highest Qualification Details</th></tr>
        <tr>
          <td style={tdr}>Highest Qualification</td><td style={tdv}>{q.highestQualification}</td>
          <td style={tdr}>Is Any Educational Gap?</td><td colSpan={2} style={tdv}>{q.isEducationalGap}</td>
        </tr>
        {q.isEducationalGap?.toUpperCase()==='YES' && (
          <tr>
            <td style={tdr}>Educational Gap (In Years)</td><td style={tdv}>{q.educationalGapYears}</td>
            <td style={tdr}>Reason of Educational Gap</td><td colSpan={2} style={tdv}>{q.educationalGapReason}</td>
          </tr>
        )}
        <tr><th colSpan={5} style={th}>{q.eligibilityQualification} Examination Details</th></tr>
        <tr>
          <td style={tdr}>Roll No. / Seat No.</td><td style={tdv}>{q.seatNo}</td>
          <td style={tdr}>No. of Attempts</td><td colSpan={2} style={tdv}>{q.noOfAttempts}</td>
        </tr>
        <tr>
          <td style={tdr}>District of Passing</td><td style={tdv}>{q.passingDistrict}</td>
          <td style={tdr}>Year of Passing</td><td colSpan={2} style={tdv}>{q.passingYear}</td>
        </tr>
        <tr><td style={tdr}>Board</td><td colSpan={4} style={tdv}>{q.board}</td></tr>
        <tr><td style={tdr}>Marks</td><td colSpan={4} style={tdv}>{q.marksObtained} / {q.marksOutOf} ({q.percentage}%)</td></tr>
        ...
        {spt.candidateID > 0 && <>
          ...
          {spt.isSportsCertificate?.toUpperCase()==='YES' && (
            <tr><td style={tdr}>Certificate Type</td><td colSpan={4} style={tdv}>{spt.certificateType}</td></tr>
          )}
        </>}
        ...
        {data.appliedColleges?.length > 0 && <>
          ...
          <tr><td colSpan={5} style={td}>
            <table><thead><tr style={{ background:'#e8e8e8' }}>
          ...
          <tbody>{(data.documents ?? []).map((doc, i) => (
            <tr key={i}>
              ...
              <td style={{ ...td, textAlign:'center' }}>
                {doc.isDocumentUploaded?.toUpperCase()==='YES' || doc.documentUploadedURL?.length>0 ? 'YES' : 'NO'}
              </td>
            </tr>
          ))}</tbody></table>
        </td></tr>
        ...
        <tr><td colSpan={5} style={td}>
          <p style={{ textAlign:'justify', margin:'4px 0', fontSize:11 }}>
            ...
            statements or information are found to be incorrect or false, I am liable to be disqualified and my
            candidature/admission may be cancelled without any notice.
          </p>
        </td></tr>
        <tr>
