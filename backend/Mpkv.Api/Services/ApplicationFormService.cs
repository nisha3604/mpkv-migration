...
namespace Mpkv.Api.Services
{
    ...
    public class ApplicationFormService : IApplicationFormService
    {
        ...
        {
            try { var p = new DynamicParameters(); p.Add("@CandidateID", candidateId); p.Add("@IsSportsCertificate", request.IsSportsCertificate); if (request.IsSportsCertificate && request.CertificateTypeID > 0) p.Add("@CertificateTypeID", (short)request.CertificateTypeID); else p.Add("@CertificateTypeID", (short)0); p.Add("@UserLoginID", userLoginId); p.Add("@IPAddress", ipAddress); p.Add("@PageCode","SportsDetails"); var result = _db.ExecuteScalar("ApplicationForm_SaveSportsDetails", p)?.ToString() ?? ""; if (result.ToUpper() == "Y") { SetRequiredDocuments(candidateId, userLoginId, ipAddress, "SportsDetails"); return new SaveSportsResponse { Success = true, Message = "Sports details saved successfully." }; } return new SaveSportsResponse { Success = false, Message = result.Length > 0 ? result : "Data has not been saved. Please try again." }; } catch (Exception ex) { return new SaveSportsResponse { Success = false, Message = ex.Message }; }
        }
